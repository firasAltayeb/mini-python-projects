...
contractions_dict = {"ain't": "are not", "aren't": "are not", "can't": "cannot", "can't've": "cannot have",
                     ...
                     "i've": "i have", "isn't": "is not", "it's": "it is", "it'd": "it would",
                     ...
                     "what's": "what is", "who's": "who is", "whom's": "whom is", "that's": "that is"}

contractions_re = re.compile('(%s)' % '|'.join(contractions_dict.keys()))


def expand_contractions(s, contract_dict=contractions_dict):
    def replace(match):
        return contract_dict[match.group(0)]

    lower_case_s = s[0].lower() + s[1:]
    expanded_s = contractions_re.sub(replace, lower_case_s)
    return expanded_s[0].upper() + expanded_s[1:]


def concurrent_run(sens):
    for j in range(1, len(sens), 2):
        expanded_sentence = expand_contractions(sens[j])
        if expanded_sentence is not None:
            print('adding {}'.format(expanded_sentence))
            with open('temp.txt', 'a+', encoding='utf-8') as new_file:
                new_file.write(sens[j - 1] + "\n")
                new_file.write(expanded_sentence + "\n")
